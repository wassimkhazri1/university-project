...
const [notesResponse, moyenneResponse] = await Promise.all([
    ...
    if (moyenneResponse.data) {
        ...
        await axios.post(`http://localhost:8080/api/moyennes/${user.id}`, null, {
            headers: { Authorization: `Bearer ${token}` }
        });
        
        // Puis on refait une requête GET pour récupérer la nouvelle moyenne
        const newMoyenneResponse = await axios.get(`http://localhost:8080/api/moyennes/${user.id}`, {
            headers: { Authorization: `Bearer ${token}` }
        });
                ...
                <table className="table table-striped table-bordered">
                    ...
                    <tbody>
                        {notes.length > 0 ? (
                            ...
                            <tr>
                                <td colSpan="7" className="text-center">Aucune note disponible</td>
                            </tr>
                        )}
                    </tbody>
                </table>
