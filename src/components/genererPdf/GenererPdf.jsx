...
const response = await axios.post(`http://localhost:8080/api/attestationEtudiant/generate/${user.id}`,user, {
   ...
   <p style={{ color: 'green' }}>Attestation générée avec succès!</p>
     ...
     setError1(error1.response?.data?.message || "Erreur lors de la génération");
...
const response1 = await axios.get(`http://localhost:8080/api/notes/exportpdf/${user.id}`, {
   ...
   setError2(error2.response1?.data?.message || "Erreur lors de la génération");
...
//https://www.linkedin.com/in/wassim-khazri-ab923a14b/ 
  return (
    ...
    <Button
      ...
      {success1 && <div style={{ color: 'green' }}>
        <i className="fas fa-check-circle"></i> Attestation générée avec succès!
      </div>}
      {error1 && <div style={{ color: 'red' }}>
    ...
    <Button
      ...
      {success2 && <div style={{ color: 'green' }}>
        <i className="fas fa-check-circle"></i> Relevé des notes générée avec succès!
      </div>}
