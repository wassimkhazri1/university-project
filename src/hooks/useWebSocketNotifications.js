...
const useWebSocketNotifications = (userId, onNewNotification) => {
      ...
      onConnect: () => {
        ...
        client.subscribe(destination, (message) => {
          try {
            ...
            console.error('❌ Erreur lors du traitement de la notification:', error, message.body);
          }
        });
        ...
        if (reconnectAttemptsRef.current < MAX_RECONNECT_ATTEMPTS) {
          ...
          if (!reconnectTimeoutRef.current) {
            reconnectTimeoutRef.current = setTimeout(() => {
              console.log(`🔄 Reconnexion ${reconnectAttemptsRef.current}/${MAX_RECONNECT_ATTEMPTS} dans ${delay}ms...`);
              connect();
  ...
  const showNotificationToast = (notification) => {
    ...
    switch (notification.type) {
        ...
        toast.custom((t) => (
          <div className={`notification-toast ${t.visible ? 'animate-enter' : 'animate-leave'}`}>
            <div className="notification-content">
              <div className="notification-icon" style={{ color: '#f59e0b', fontSize: '24px' }}>
                ⚠️
              </div>
              <div className="notification-text">
                ...
                {notification.date && (
                  <p style={{ fontSize: '12px', color: '#999' }}>
                    Date: {new Date(notification.date).toLocaleDateString('fr-FR')}
                  </p>
                )}
              </div>
              <button 
      ...
      case 'NOTE_ADDED':
        toast.success(`📝 ${notification.message || 'Nouvelle note ajoutée'}`, toastOptions);
        break;
...
const fetchPendingNotifications = useCallback(async () => {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:8080/api/notifications/pending`, {
      headers: {
    ...
    if (response.ok) {
      ...
      console.log(`Récupéré ${pendingNotifications.length} notifications en attente`);
    } else if (response.status === 401) {
      console.log('Non authentifié, impossible de récupérer les notifications');
    }
  } catch (error) {
  ...
  const sendNotificationRead = useCallback(async (notificationId) => {
  ...
  const sendTestNotification = useCallback(() => {
    if (!clientRef.current || !isConnectedRef.current) {
      console.log('WebSocket non connecté, impossible d\'envoyer une notification de test');
      return;
    }
